...
//|                                                                   |
//|  Magic: 481370 (jedinstven, ne konfliktira s FRANKENSTEIN)       |
//+------------------------------------------------------------------+
#property copyright "BOUNCE_Cla v2.0 — Mean Reversion EA (2026-03-12)"
...
input double MFE0_Pct = 0.95; // Phase 0: lock %
...
input group "=== OPĆE ==="
input ulong MagicNumber = 481370;
...
int OnInit()
{
    ...
    { Print("BOUNCE: Handle FAILED"); return INIT_FAILED; }
    ...
    Print("╔══════════════════════════════════════════════════╗");
    Print("║      BOUNCE_Cla v2.0 — Mean Reversion EA        ║");
    Print("╠══════════════════════════════════════════════════╣");
...
void CheckDailyReset()
{
    ...
    if(now.day!=last.day)
    {
        ...
        Print("BOUNCE: Novi dan | P&L: ",NormalizeDouble(cb-g_dailyStartBal,2));
        g_dailyStartBal=cb; g_lastDayReset=TimeCurrent();
        if(g_tradingHalted&&g_haltReason=="DAILY")
        { g_tradingHalted=false; g_haltReason=""; Print("BOUNCE: Daily reset — nastavlja"); }
    }
}
...
bool CheckFTMOLimits()
{
    ...
    if(eq<floor)
    {
        if(!g_tradingHalted){
            ...
            Print("!!! BOUNCE FTMO: TOTAL DD — ZAUSTAVLJENO !!!");
    ...
    if(dailyLoss>dailyLimit)
    {
        if(!g_tradingHalted){
            ...
            Print("!!! BOUNCE FTMO: DAILY LIMIT — zaustavljeno do ponoći !!!");
        }
        return false;
    }
...
void QueueTrade(int direction)
{
    ...
    if(scale>1.0) Print("BOUNCE: Cascade ×",DoubleToString(scale,2)," lot=",g_pending.lots);
}
...
void ProcessPending()
{
    ...
    bool ok=(dir==1)?trade.Buy(lots,_Symbol,price,sl,0,"BOUNCE_Cla")
                    :trade.Sell(lots,_Symbol,price,sl,0,"BOUNCE_Cla");
    if(ok)
    {
        ...
        Print("BOUNCE ",(dir==1?"BUY":"SELL")," #",ticket," @ ",price," SL=",sl," lot=",lots,
              " RSI=",RSI_Period,"(",RSI_Oversold,"/",RSI_Overbought,")");
    }
    else Print("BOUNCE: OPEN FAILED — ",trade.ResultRetcodeDescription());
...
void ManagePosition()
{
    ...
    { Print("BOUNCE: Petkom zatvaranje #",g_pos.ticket); trade.PositionClose(g_pos.ticket); return; }
...
void OnTick()
{
    ...
    Print("BOUNCE: Signal ",(sig==1?"BUY":"SELL")," RSI7 crossover");
    QueueTrade(sig);
}
