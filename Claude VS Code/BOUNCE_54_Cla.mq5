//+------------------------------------------------------------------+
//|                                                BOUNCE_54_Cla.mq5 |
//|            *** Bounce 54 — ChatGPT mean reversion EA ***         |
//|              Created: 12.03.2026 (Zagreb)                         |
//|              Upgrade: 12.03.2026 — v2.0: MFE + SL + RSI optimum |
//|              Fixed:   13.03.2026 — v54: pip/trailing repair     |
//|                                                                   |
...
//|                                                                   |
//|  OPTIMIZACIJA v2.0 (4224 MFE combos + SL/RSI sweep):            |
//|    Winner MFE median=137 pips → P2 mora biti 150 (ne 80!)       |
//|    91.5% losers imaju MFE=0 → isti pattern kao FRANKENSTEIN      |
//|    SL 400-440 daje 100x veći balance od 750-790 (agresivniji lot)|
//|    RSI OS=40/OB=60 → +87% više trejdova, isti DD                |
//|                                                                   |
//|  BACKTEST v2.0 (3god, XAUUSD M5, $10k start, 0.5% risk):       |
//|    v1.0: RSI7 OS=30/OB=70 SL=750-790: T=12096 WR=91.6% $16M   |
//|    v2.0: RSI7 OS=40/OB=60 SL=400-440: T=22682 WR=85.9% $23.3T  |
//|    + M15+H1 cascade (×1.8): astronomski potencijal              |
//|    v2.1: sporiji trailing + ispravan pip/lot model za live      |
//|                                                                   |
//|  KOMPLEMENTARAN s FRANKENSTEIN_Cla.mq5:                         |
//|    FRANKENSTEIN = momentum (trendovi)                            |
//|    BOUNCE = mean reversion (oscilacije)                          |
//|    Oba rade na istom accountu (ne hedging — isti smjer max 1 pos)|
//|                                                                   |
//|  Magic: 540054 (jedinstven, ne konfliktira s originalnim Bounce) |
//+------------------------------------------------------------------+
#property copyright "BOUNCE_54_Cla — ChatGPT tuned live variant (2026-03-13)"
...
// v2.0 optimizacija: 4224 kombinacija na RSI7 signalima
// Winner MFE median=137 pips → P2 mora biti 150 (ne 80 kao FRANKENSTEIN!)
// P1=99% jer RSI7 winners imaju visok MFE — ne pušta se previše
input int    MFE0_Act = 0;    // Phase 0 disabled: preagresivno za live
input double MFE0_Pct = 0.00; // Phase 0: lock %
input int    MFE1_Act = 80;   // Phase 1: aktivacija (pips) nakon stvarnog pomaka
input double MFE1_Pct = 0.35; // Phase 1: lock dio profita
input int    MFE2_Act = 180;  // Phase 2: aktivacija (pips)
input double MFE2_Pct = 0.65; // Phase 2: jači trailing tek nakon većeg MFE

input group "=== RISK & SL ==="
input double RiskPercent = 0.5;   // Risk % od initial balance
input int    SL_Min      = 400;   // SL min pips [v2.0 optimum: 400]
input int    SL_Max      = 440;   // SL max pips [v2.0 optimum: 440]
input bool   UseInitialBalance = true; // true=stabilan lot | false=compounding
input int    MinHoldSeconds = 90; // Nemoj micati trailing odmah nakon entryja
...
input bool   FTMO_Active     = true;  // Aktiviraj FTMO zaštitu
input bool   FTMO_CloseOnFriday = false; // Zatvori petkom (Standard account)
...
input group "=== OPĆE ==="
input ulong MagicNumber = 540054;
...
int OnInit()
{
    ...
    else
    {
        ...
        g_pipSize=(digits==3||digits==5) ? SymbolInfoDouble(_Symbol,SYMBOL_POINT)*10.0
    ...
    { Print("BOUNCE_54: Handle FAILED"); return INIT_FAILED; }
    ...
    Print("╔══════════════════════════════════════════════════╗");
    Print("║       BOUNCE_54_Cla — ChatGPT variant          ║");
    Print("╠══════════════════════════════════════════════════╣");
...
void CheckDailyReset()
{
    ...
    if(now.day!=last.day)
    {
        ...
        Print("BOUNCE_54: Novi dan | P&L: ",NormalizeDouble(cb-g_dailyStartBal,2));
        g_dailyStartBal=cb; g_lastDayReset=TimeCurrent();
        if(g_tradingHalted&&g_haltReason=="DAILY")
        { g_tradingHalted=false; g_haltReason=""; Print("BOUNCE_54: Daily reset — nastavlja"); }
    }
}
...
bool CheckFTMOLimits()
{
    ...
    if(eq<floor)
    {
        if(!g_tradingHalted){
            ...
            Print("!!! BOUNCE_54 FTMO: TOTAL DD — ZAUSTAVLJENO !!!");
    ...
    if(dailyLoss>dailyLimit)
    {
        if(!g_tradingHalted){
            ...
            Print("!!! BOUNCE_54 FTMO: DAILY LIMIT — zaustavljeno do ponoći !!!");
        }
        return false;
    }
...
double CalcLots(int slPips, double scaleMult=1.0)
{
    ...
    if(slDistance<=0 || tv<=0 || ts<=0) return 0;
    double moneyPerLot=(slDistance/ts)*tv;
...
void QueueTrade(int direction)
{
    ...
    if(scale>1.0) Print("BOUNCE_54: Cascade ×",DoubleToString(scale,2)," lot=",g_pending.lots);
}
...
void ProcessPending()
{
    ...
    if(ShouldCloseFriday() || !IsTradingWindow()){g_pending.active=false;return;}
    ...
    double sl=NormalizeDouble((dir==1)?price-g_pending.slPips*g_pipSize:price+g_pending.slPips*g_pipSize,digits);

    bool ok=(dir==1)?trade.Buy(lots,_Symbol,price,sl,0,"BOUNCE_54_Cla")
                    :trade.Sell(lots,_Symbol,price,sl,0,"BOUNCE_54_Cla");
    if(ok)
    {
        ...
        Print("BOUNCE_54 ",(dir==1?"BUY":"SELL")," #",ticket," @ ",price," SL=",sl," lot=",lots,
              " RSI=",RSI_Period,"(",RSI_Oversold,"/",RSI_Overbought,")");
    }
    else Print("BOUNCE_54: OPEN FAILED — ",trade.ResultRetcodeDescription());
...
void ManagePosition()
{
    ...
    { Print("BOUNCE_54: Petkom zatvaranje #",g_pos.ticket); trade.PositionClose(g_pos.ticket); return; }
    ...
    double mfeNow=(pt==POSITION_TYPE_BUY)?(curPrice-g_pos.entry)/g_pipSize:(g_pos.entry-curPrice)/g_pipSize;
    ...
    if(MinHoldSeconds>0 && (TimeCurrent()-g_pos.openTime)<MinHoldSeconds)
        return;
    ...
    if(lock>0)
    {
        double ns=(pt==POSITION_TYPE_BUY)?g_pos.entry+lock*g_pipSize:g_pos.entry-lock*g_pipSize;
...
void OnTick()
{
    ...
    Print("BOUNCE_54: Signal ",(sig==1?"BUY":"SELL")," RSI7 crossover");
    QueueTrade(sig);
}
