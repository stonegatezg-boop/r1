//+------------------------------------------------------------------+
//|                                             FUSION_FTMO_Cla.mq5  |
//|          *** FUSION FTMO v2.0 — IMPULSE × CALF_C (FTMO) ***     |
//|          Created: 12.03.2026 20:30 (Zagreb)                      |
//|          Upgrade: 12.03.2026 — v2.0: M15+H1 kaskada,            |
//|                   Tri-Phase MFE, FTMO_UseInitialBalance removed  |
//|                                                                   |
...
//|  Magic: 372824                                                   |
//+------------------------------------------------------------------+
#property copyright "FUSION_FTMO_Cla v2.0 — IMPULSE × CALF_C (2026-03-12)"
...
input group "=== OPĆE ==="
input ulong MagicNumber = 372824;
...
int OnInit()
{
    ...
    Print("╔══════════════════════════════════════════════════════╗");
    Print("║  FUSION_FTMO_Cla v2.0 — IMPULSE × CALF_C           ║");
    Print("╠══════════════════════════════════════════════════════╣");
    Print("║  Signal:  ", sigName);
    Print("║  Smjer:   ", (Direction==DIR_BUY_ONLY?"BUY ONLY":(Direction==DIR_SELL_ONLY?"SELL ONLY":"BOTH")));
    ...
    Print("║  Friday:  ", FTMO_CloseOnFriday?"Zatvori u "+IntegerToString(FTMO_FridayHour)+":00h":"Swing (ne zatvara)");
...
void CheckDailyReset()
{
    ...
    if(now.day!=last.day)
    {
        ...
        Print("FUSION_FTMO: Novi dan | P&L: ",NormalizeDouble(cb-g_dailyStartBal,2));
        g_dailyStartBal=cb; g_lastDayReset=TimeCurrent();
        if(g_tradingHalted&&g_haltReason=="DAILY")
        { g_tradingHalted=false; g_haltReason=""; Print("FUSION_FTMO: Daily reset — nastavlja"); }
    }
}
...
bool CheckFTMOLimits()
{
    ...
    if(eq<floor)
    {
        if(!g_tradingHalted){
            ...
            Print("!!! FUSION_FTMO: TOTAL DD — ZAUSTAVLJENO !!!");
    ...
    if(dailyLoss>dailyLimit)
    {
        if(!g_tradingHalted){
            ...
            Print("!!! FUSION_FTMO: DAILY LIMIT — zaustavljeno do ponoći !!!");
        }
        return false;
    }
...
void QueueTrade(int direction)
{
    ...
    if(scale>1.0) Print("FUSION_FTMO: Cascade ×",DoubleToString(scale,2)," lot=",g_pending.lots);
}
...
void ProcessPending()
{
    ...
    bool ok=(dir==1)?trade.Buy(lots,_Symbol,price,sl,0,"FUSION_FTMO_Cla")
                    :trade.Sell(lots,_Symbol,price,sl,0,"FUSION_FTMO_Cla");
    if(ok)
    {
        ...
        Print("FUSION_FTMO [",sn,"] ",(dir==1?"BUY":"SELL")," #",ticket," @ ",price," SL=",sl," lot=",lots);
    }
    else Print("FUSION_FTMO: OPEN FAILED — ",trade.ResultRetcodeDescription());
...
void ManagePosition()
{
    ...
    { Print("FUSION_FTMO: Petkom zatvaranje #",g_pos.ticket); trade.PositionClose(g_pos.ticket); return; }
...
void OnTick()
{
    ...
    Print("FUSION_FTMO: Signal ",(sig==1?"BUY":"SELL"));
    QueueTrade(sig);
}
