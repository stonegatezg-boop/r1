...
//|   + Stealth Mode v2.0                                            |
//|   + Enhanced v3.0: Trend Filter, ATR SL, Partial TP, Dynamic Trail|
//|                                                                  |
//|   Version 3.0 - 2026-02-20                                       |
//+------------------------------------------------------------------+
#property strict
#property version   "3.00"
#property copyright "KymcoDe v3.0"
...
input bool   StrictTrendFilter  = true;     // Samo s trendom

input group "=== VOLATILITY FILTER ==="
input double MinATR             = 1.0;      // Minimum ATR za trade
input double MaxATR             = 15.0;     // Maximum ATR (izbjegni extreme vol)
...
int OnInit()
{
   ...
   Print("=== KymcoDe v3.0 ENHANCED + STEALTH MODE ===");
...
int GetH1TrendDirection()
{
   ...
   if(price > ema[0] + threshold && ema[0] > ema[1]) return 1;   // Bullish
   if(price < ema[0] - threshold && ema[0] < ema[1]) return -1;  // Bearish
   return 0;  // Neutral
}
...
int GetCombinedTrend()
{
   ...
   {
      // Barem jedan mora biti pozitivan
      if(h1Trend == 1 || hullTrend == 1) return 1;
      if(h1Trend == -1 || hullTrend == -1) return -1;
      return 0;
   }
}
...
void CheckDonchianSignal()
{
   ...
   // Breakout BUY - samo ako trend podržava
   if(close[0] > upper)
   {
      if(!UseTrendFilter || trend >= 0 || (!StrictTrendFilter && trend != -1))
      {
         QueueTrade(ORDER_TYPE_BUY);
      }
   }
   // Breakout SELL - samo ako trend podržava
...
void QueueTrade(ENUM_ORDER_TYPE type)
{
   ...
   if(UseStealthMode)
   {
      ...
      Print("KymcoDe: Trade queued (", (type == ORDER_TYPE_BUY ? "BUY" : "SELL"),
            "), ATR=", DoubleToString(atr, 2), ", delay ", g_pendingTrade.delaySeconds, "s");
   }
   else
   {
...
void ExecuteTrade(ENUM_ORDER_TYPE type, double lot, double sl, double tp, double partialTP)
{
   ...
   if(UseStealthMode)
   {
      ok = (type == ORDER_TYPE_BUY)
           ? trade.Buy(lot, _Symbol, entry, 0, 0, "KymcoDe v3")
           : trade.Sell(lot, _Symbol, entry, 0, 0, "KymcoDe v3");
   }
   else
   {
      ok = (type == ORDER_TYPE_BUY)
           ? trade.Buy(lot, _Symbol, entry, sl, tp, "KymcoDe v3")
           : trade.Sell(lot, _Symbol, entry, sl, tp, "KymcoDe v3");
   }
   ...
   if(ok)
   {
      ...
      Print("KymcoDe v3: Opened ", (type == ORDER_TYPE_BUY ? "BUY" : "SELL"),
...
void OnTick()
{
   ...
   // Samo Donchian breakout (konzistentna strategija)
   CheckDonchianSignal();
}
