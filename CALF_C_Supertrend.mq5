...
//+------------------------------------------------------------------+
#property copyright "CALF C - Supertrend + Stealth (2026-02-20)"
#property version   "2.00"
#property strict

#include <Trade\Trade.mqh>

//--- Stealth position tracking
...
input int      SLDelayMin       = 7;        // Min SL delay (sec)
input int      SLDelayMax       = 13;       // Max SL delay
input double   LargeCandleATR   = 3.0;      // Skip large candles (ATR mult)

input group "=== TRAILING POSTAVKE ==="
input bool     UseTrailing      = true;     // Enable trailing stop
input int      TrailActivatePips = 500;     // Pips to activate BE
input int      TrailBEPipsMin   = 33;       // Min pips above BE
input int      TrailBEPipsMax   = 38;       // Max pips above BE
input int      TrailLevel2Pips  = 1000;     // Level 2 pips
input int      TrailLevel2SLMin = 150;      // Min pips to lock at L2
input int      TrailLevel2SLMax = 200;      // Max pips to lock at L2
...
int OnInit()
{
    ...
    Print("=== CALF C v2.0 (Supertrend ", STperiod, ",", STmultiplier, ") + STEALTH MODE ===");
...
void ManageStealthPosition()
{
    ...
    if(UseStealthMode && currentSL == 0 && g_stealthPos.intendedSL != 0)
    {
        if(TimeCurrent() >= g_stealthPos.openTime + g_stealthPos.delaySeconds)
        {
            if(trade.PositionModify(g_stealthPos.ticket, NormalizeDouble(g_stealthPos.intendedSL, digits), 0))
                Print("CALF_C STEALTH: SL set #", g_stealthPos.ticket);
        }
    }
    ...
    if(g_stealthPos.stealthTP > 0)
    {
        ...
        if(tpHit)
        {
            if(trade.PositionClose(g_stealthPos.ticket))
                Print("CALF_C STEALTH: TP hit #", g_stealthPos.ticket);
            g_stealthPos.active = false;
            return;
        }
    }
    ...
    if(UseTrailing && g_stealthPos.trailLevel < 2 && profitPips >= TrailLevel2Pips && currentSL > 0)
    {
        ...
        if(shouldModify && trade.PositionModify(g_stealthPos.ticket, newSL, 0))
        {
            ...
            Print("CALF_C TRAIL L2: SL -> +", randomL2, " pips");
        }
        return;
    }
    ...
    if(UseTrailing && g_stealthPos.trailLevel < 1 && profitPips >= TrailActivatePips && currentSL > 0)
    {
        ...
        if(shouldModify && trade.PositionModify(g_stealthPos.ticket, newSL, 0))
        {
            ...
            Print("CALF_C TRAIL BE: SL -> BE+", g_stealthPos.randomBEPips, " pips");
        }
    }
}
...
void QueueTrade(ENUM_ORDER_TYPE type)
{
    ...
    if(UseStealthMode)
    {
        ...
        Print("CALF_C: Trade queued, delay ", g_pendingTrade.delaySeconds, "s");
    }
    else
    {
...
void ExecuteTrade(ENUM_ORDER_TYPE type, double lot, double sl, double tp)
{
    ...
    if(UseStealthMode)
        ok = (type == ORDER_TYPE_BUY) ?
            trade.Buy(lot, _Symbol, price, 0, 0, "CALF_C") :
            trade.Sell(lot, _Symbol, price, 0, 0, "CALF_C");
    else
        ok = (type == ORDER_TYPE_BUY) ?
            trade.Buy(lot, _Symbol, price, sl, tp, "CALF_C BUY") :
            trade.Sell(lot, _Symbol, price, sl, tp, "CALF_C SELL");
    ...
    if(ok)
    {
        ...
        if(UseStealthMode)
        {
            ...
            Print("CALF_C STEALTH: Opened #", ticket, ", SL delay ", slDelay, "s");
        }
        Print("CALF_C ", (type == ORDER_TYPE_BUY ? "BUY" : "SELL"), ": ", lot, " @ ", price);
    }
}
...
void OnTick()
{
    ...
    if(buySignal) { Print("CALF_C BUY SIGNAL (Supertrend flip)"); QueueTrade(ORDER_TYPE_BUY); }
    else if(sellSignal) { Print("CALF_C SELL SIGNAL (Supertrend flip)"); QueueTrade(ORDER_TYPE_SELL); }
}
//+------------------------------------------------------------------+
